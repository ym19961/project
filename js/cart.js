$(document).ready(function() {
    ...
    $("#btn_settlement").click(function(){
    	if($("#priceTotal").text()=='0.00'){
    		return false;
    	}
    	var timestamp=new Date().getTime();//获得时间戳
    	var userid = $("#userid").val();
    	var totalprice = $("#priceTotal").text();
    	var length = 0;
    	var dishes=new Array();
    	var jsonstr="[]";
    	var infoarray=eval('('+jsonstr+')');
    	$(cartTable).find("tr:gt(0)").each(function(i,v) {
            if ($(this).find(":checkbox").prop("checked") == true) {
                 var dishid = $(this).children().find("#dishid").val();
                 var num = $(this).children().next().find("#num").val();
            	 var info={
            			 'dishid':dishid,
            			 'num':num
            	 };
    	...
    	$.post("../ShoppingCart/processOrderInfo",{userid:userid,timestamp:timestamp,infoarray:JSON.stringify(infoarray),totalprice:totalprice},function(data){
    		if(data="1"){
    			location.href = "../ShoppingCart/confirmation";
    		}
    		else{
    			location.reload();
    		}
        ...
        $(this).click(function() {
            ...
            if ($(window.event.srcElement).hasClass("minus")) {
                ...
                if (confirm("确定要从购物车中删除此产品？")) {
                	var dishid = $(this).children().attr("id");
                	$(this).remove();
                	$.post("../ShoppingCart/deleteItem",{dishid:dishid},function(data){
                		location.reload();
                	});
                }
            }
