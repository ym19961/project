...
<script src="<%=path %>/js/jquery.vide.min.js"></script>
	<div style="background-image:url(<%=path %>/video/cook.jpg)">
		<!-- banner -->
		<div class="banner">
			<div class="container">
				<div class="header">
											...
											<c:otherwise>
											 	<div class="dropdown" id="haschild">
												  ...
												  <ul id="list" class="dropdown-menu" aria-labelledby="dLabel" style="background-color:#111">
												    ...
												    <li><a href="<%=path %>/User/userinfo">账户设置</a></li>											    		
													<li><a href="<%=path %>/Logout/logout">注销登录</a></li>
												  </ul>
												</div>
											</c:otherwise>
										</c:choose>
					...
					<div class="col-md-4 banner-grad">
						<div class="banner-grad-img">
							<a href="../Menu/SinglePage?dishid=${item.dishid }&dname=${item.dname}
											...
											&dimg=${item.dimg }&xiaoliang=${item.xiaoliang }"><img src="<%=path %>/img/${item.dname }.jpg" alt="" /></a>
							<h4>${item.dname }</h4>
						</div>
					</div>
				</c:forEach>
